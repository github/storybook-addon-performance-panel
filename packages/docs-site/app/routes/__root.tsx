      ...
      <body>
        <header className="site-header">
          <Link to="/" className="site-header-home">
            <svg width="24" height="24" viewBox="0 0 98 96" fill="currentColor" aria-hidden="true">
              <path
                fillRule="evenodd"
                clipRule="evenodd"
          ...
          <nav className="site-header-nav" aria-label="External links">
            <a
              ...
              {NAV.map(group => (
                <NavList.Group key={group.section}>
                  ...
                  {group.items.map(item => {
                    if ('href' in item) {
                      return (
                        <NavList.Item key={item.href} href={item.href}>
                          {item.label} ↗
                        </NavList.Item>
                      )
                    }
                    ...
                    if (!item.children?.length) {
                      return (
                        <NavList.Item key={item.to} href={item.to} aria-current={isActive ? 'page' : undefined}>
                          {item.label}
                        </NavList.Item>
                      )
                    }
                    return (
                      <NavList.Item key={item.to} defaultOpen={isActive} aria-current={isActive ? 'page' : undefined}>
                        {item.label}
                        <NavList.SubNav>
...
export const Route = createRootRoute({
  ...
  notFoundComponent: () => (
    <div className="mdx-content" style={{padding: '3rem', textAlign: 'center'}}>
      <h1>404 — Page not found</h1>
      <p>
        <span>{"The page you're looking for doesn't exist."}</span> <Link to="/">Go home</Link>.
      </p>
    </div>
  ),
  head: () => ({
