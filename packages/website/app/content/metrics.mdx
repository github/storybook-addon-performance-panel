...
<div className="page-toc">
  <div className="page-toc-title">Metric categories</div>
  <ul>
    <li><a href="#key-thresholds">Key thresholds</a></li>
    <li><a href="#frame-timing">Frame timing</a></li>
    <li><a href="#input-responsiveness">Input responsiveness</a></li>
    <li><a href="#main-thread-health">Main thread health</a></li>
    <li><a href="#long-animation-frames">Long animation frames</a></li>
    <li><a href="#element-timing">Element timing</a></li>
    <li><a href="#layout-stability">Layout stability</a></li>
    <li><a href="#react-performance">React performance</a></li>
    <li><a href="#memory-and-resources">Memory and resources</a></li>
  </ul>
</div>
...
| Metric | What it tells you |
|--------|-------------------|
| **INP** | Interaction to Next Paint — the p98 of worst interactions. A Core Web Vital. |
...
- **Wait (input delay)** — time before JavaScript starts processing. High values mean the main thread was blocked.
- **JS (processing time)** — time spent in event handlers. High values mean expensive handler logic.
- **Paint (presentation delay)** — time from handler completion to the visual update. High values mean expensive rendering.
...
| **Total blocking time** | Sum of `(duration − 50ms)` for all long tasks. Aligns with the Lighthouse TBT metric. |
...
| **Script attribution** | Which scripts caused slow frames — including source URL, function name, and invoker type. |
...
React-specific metrics from the [React Profiler API](https://react.dev/reference/react/Profiler). Only available when using the default (React) addon entry — hidden when using `./universal`.
...
| **Render cascades** | Nested updates during the commit phase — typically `setState` inside `useLayoutEffect`. Very expensive. |
