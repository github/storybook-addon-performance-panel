...
Add the addon to your Storybook `addons` array. This registers the panel UI in the manager.
...
Use `definePreview` to compose the addon's preview annotations (decorator). This enables the CSF factory API (`preview.meta()`, `meta.story()`):
...
Use the `./universal` subpath — it collects all browser-level metrics without a React dependency.
...
```ts
// .storybook/main.ts
import type {StorybookConfig} from '@storybook/html-vite' // or vue-vite, svelte-vite, etc.
