  ...
  start(): void {
    ...
    this.#observer = new MutationObserver(mutations => {
      const processMutations = () => {
        this.#overheadTelemetry?.recordScan('style.mutation-records', mutations.length)
      ...
      if (this.#overheadTelemetry) {
        this.#overheadTelemetry.measureCallback('style.mutations', processMutations)
      } else {
        processMutations()
      }
    })
    ...
    this.#sampleInterval = setInterval(() => {
      ...
      if (this.#overheadTelemetry) {
        this.#overheadTelemetry.measureCallback('style.sample', sampleMutations)
      } else {
        sampleMutations()
      }
